{ pkgs ? import <nixpkgs> {} }:

let
    lib = (import ./default.nix) { inherit pkgs; };
    testingSpec = pkgs.fetchurl {
        url = "https://raw.githubusercontent.com/OAI/OpenAPI-Specification/9df68a1dafd467d9fdbf68653f351b860b4ec6e5/examples/v3.0/petstore.yaml";
        sha256 = "sha256-WYE2y5BOF+ju6tUa4z3Y1AH9/0VdLXTzhpxKpfJ0ImY=";
    };

    createCommandForGeneratorName =
        generator-name:
            let
                generated = (lib.getGenerator generator-name) { specification = testingSpec; };
            in
            builtins.concatStringsSep "\n" [
                ''
                    echo "Symlinking OpenAPI generated library: ${generator-name}"
                    ln -s ${generated.sourceCode} $out/all/sources/${generator-name}
                ''
                (
                    if generated.dontBuildBinaryReason == null then ''
                        ln -s ${generated.binary} $out/all/binaries/${generator-name}
                    ''
                    else ''
                        ln -s ${pkgs.writeText "nix-openapi-generator-test-${generator-name}-unsupported-binary.txt" generated.dontBuildBinaryReason} $out/all/binaries/${generator-name}-not-supported.txt
                        echo "${generator-name} doesn't support building a binary. Reason: ${builtins.replaceStrings ["\n"] ["\n  "] generated.dontBuildBinaryReason}"
                    ''
                )
            ];
    commands =
        builtins.map
            createCommandForGeneratorName
            (["go"]  ++ builtins.attrNames lib.generators);  #builtins.mapAttrs createCommandForAttr lib.generators;
    buildScript = builtins.concatStringsSep "\n" commands;

    specificTests = {
        haskell-http-client = (lib.getGenerator "haskell-http-client" {
            specification = testingSpec;
            configuration = {
                cabalPackage = "petstore-openapi-autogenerated";
            };
        }).binary;
    };
in

pkgs.stdenv.mkDerivation {
    name = "nix-openapi-generator-test";
    dontUnpack = true;
    buildPhase = ''
        mkdir -p $out/all/binaries/
        mkdir -p $out/all/sources/
        ${buildScript}

        mkdir -p $out/specific
        cp -r ${specificTests.haskell-http-client} $out/specific/haskell-http-client
    '';
}
